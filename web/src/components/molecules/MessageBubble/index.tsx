...
// src/components/molecules/MessageBubble/index.tsx
...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const getStatusIcon = () => {
    switch (status) {
      case 'sending':
        return <span className="text-white/70 animate-pulse">🕐</span>;
      case 'sent':
        return <span className="text-white/70">✓</span>;
      case 'delivered':
        return <span className="text-white/70">✓✓</span>;
      case 'read':
        return <span className="text-blue-300">✓✓</span>; // Blue double-tick!
      default:
        return null;
    }
  };
  ...
  return (
    <div className={`flex gap-2 mb-3 ${isSent ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`flex flex-col max-w-[75%] ${isSent ? 'items-end' : 'items-start'}`}>
        {/* Sender name for group chats */}
        {!isSent && isGroup && senderName && (
          <p className="text-xs font-semibold text-purple-600 mb-1 ml-3">{senderName}</p>
        )}
        ...
        >
            ...
            <div className="mb-2">
              {mediaType === 'image' && (
                <img
                  ...
                  onClick={() => onMediaClick && onMediaClick(mediaUrl, 'image')}
                />
              )}
              {mediaType === 'video' && (
                <video
                  ...
                  controls
                  className="rounded-lg max-w-xs max-h-64"
                  onClick={() => onMediaClick && onMediaClick(mediaUrl, 'video')}
                />
              )}
            </div>
          )}
          ...
          {/* Text content */}
          {text && (
            <p className={`break-words whitespace-pre-wrap ${isSent ? 'text-white' : 'text-gray-900'}`}>
              {text}
            </p>
          )}
          ...
          {/* Timestamp and status */}
          <div className={`flex items-center gap-1 mt-1 ${isSent ? 'justify-end' : 'justify-start'}`}>
            <span className={`text-[10px] ${isSent ? 'text-white/70' : 'text-gray-400'}`}>
              {formatMessageTime(timestamp)}
            </span>
            {isSent && status && <span className="flex-shrink-0 text-xs">{getStatusIcon()}</span>}
          </div>
        </div>
      </div>
    </div>
